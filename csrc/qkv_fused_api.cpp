...
void run_qkv_fusion_lightweight(QKVFusedParams &params, cublasHandle_t cublas_handle, cudaStream_t stream);
...
// Lightweight forward: GEMM + Bias only (split/transpose in PyTorch)
torch::Tensor qkv_fused_forward_lightweight(
    torch::Tensor hidden_states,      // [batch, seqlen, hidden_dim]
    torch::Tensor qkv_fused_weight,   // [hidden_dim, qkv_out_dim]
    c10::optional<torch::Tensor> qkv_fused_bias,
    int64_t num_q_heads,
    ...
    // Flatten input to 2D for optimal GEMM performance
    const int M = batch_size * seqlen;
    const int N = qkv_out_dim;
    const int K = hidden_dim;
    
    // Ensure input is contiguous and 2D
    auto hidden_states_2d = hidden_states.view({M, K});
    
    // Allocate output tensor as 3D directly (avoid view overhead)
    auto options = torch::TensorOptions()
        .dtype(torch::kFloat16)
        .device(hidden_states.device());
    auto qkv_output = torch::empty({batch_size, seqlen, N}, options);
    
    // Set up parameters
    QKVFusedParams params;
    
    // Input pointers (now guaranteed 2D layout)
    params.hidden_states_ptr = hidden_states_2d.data_ptr();
    params.qkv_fused_weight_ptr = qkv_fused_weight.data_ptr();
    params.qkv_fused_bias_ptr = qkv_fused_bias.has_value() ? qkv_fused_bias.value().data_ptr() : nullptr;
    
    // Use output tensor as workspace (no extra allocation needed!)
    ...
    // Launch lightweight kernel (GEMM + bias only)
    run_qkv_fusion_lightweight(params, cublas_handle, stream);
    
    // Output is already in correct shape [batch, seqlen, qkv_out_dim]
    return qkv_output;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    */
    ...
    m.def("qkv_fused_forward_lightweight", &qkv_fusion::qkv_fused_forward_lightweight,
          "QKV Fused Forward Pass (Lightweight: GEMM+Bias only, Phase 3)",
          py::arg("hidden_states"),
          py::arg("qkv_fused_weight"),
          py::arg("qkv_fused_bias") = py::none(),
          py::arg("num_q_heads"),
